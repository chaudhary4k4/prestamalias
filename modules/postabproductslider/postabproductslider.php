...
//            $this->context->controller->addCSS(($this->_path).'animate.min.css', 'all');
	...
	public function hookblockPosition3($params) {
			...
			if(Configuration::get($this->name . '_show_feature')) {
				$productTabslider[] = array('id'=>'feature_product','name' => $this->l('Feature Products'), 'productInfo' =>  $featureProduct);
			}
	 ...
	 $tabEffect = array();
		...
		<form action="'.$_SERVER['REQUEST_URI'].'" method="post">
                  <fieldset>
                    ...
                    <label>'.$this->l('Width Item: ').'</label>
                    <div class="margin-form">
                            <input type = "text"  name="p_width" value ='. (Tools::getValue('p_width')?Tools::getValue('p_width'): Configuration::get($this->name.'_p_width')).' ></input>
                    </div>
					<label>'.$this->l('Height Item: ').'</label>
                    <div class="margin-form">
                            <input type = "text"  name="p_height" value ='. (Tools::getValue('p_height')?Tools::getValue('p_height'): Configuration::get($this->name.'_p_height')).' ></input>
                    </div>
