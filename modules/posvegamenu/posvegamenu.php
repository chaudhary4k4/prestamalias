...
class Posvegamenu extends Module {
    ...
    public function getStaticBlockContent($blockId = NULL, $task = NULL) {
        ...
        if (count($staticBlock) > 0) {
            ...
            $description = str_replace('/pos_malias/',__PS_BASE_URI__,$description);
            $html .= $description;
        }
        if ($task == 'item') {
	...
	public function drawCustomMenuItem($category, $level = 0, $last = false, $item, $lang_id) {
      ...
      //  $files = scandir(_PS_CAT_IMG_DIR_);
       ...
       //                 $html[] = '<span><img class="cate-thumb" src="'.$this->context->link->getMediaLink(_THEME_CAT_DIR_.$file).'" alt= ""/></span>';
		//}
    ...
    public function drawCustomMenuBlock($blockId, $bc) {
        ...
        $blockHtml = str_replace('/pos_malias/',__PS_BASE_URI__,$bc['description']);
        $drawPopup = $blockHtml;
        if ($drawPopup) {
