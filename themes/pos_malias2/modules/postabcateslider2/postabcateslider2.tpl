...
$(document).ready(function() {
	...
	$("ul.tab2_cates li").click(function() {
		...
		$(".tab2_category").hide();
		$(".tab2_title").hide();
		$(".tab2_category").removeClass("animate1 {$tab_effect}");
	...
	<div class="container-tab">
		<div class="container-inner">
			<div class="tab-category">
				...
				<div class="pos_tab tab_cate2">

					<div class ='cate_title'>
						{foreach from=$productCates item=productCate name=posTabCategory}
						 <div id="tabtitle2_{$productCate.id}" class="tab2_title"> 		
								<h2>
					...
					{$count=0}
					{foreach from=$productCates item=productCate name=posTabCategory}
						<li data-title="tabtitle2_{$productCate.id}" rel="tab2_{$productCate.id}" {if $count==0} class="active"  {/if} > {$productCate.name}</li>
							{$count= $count+1}
					{/foreach}	
					</ul>
				</div>	
				...
				{foreach from=$productCates item=productCate name=posTabCategory}
					...
					<div id="tab2_{$productCate.id}" class="tab2_category">
						<div class="row">
										...
										<div class="item-inner">
											<a class ="bigpic_{$product.id_product}_tabcategory product_image" href="{$product.link|escape:'html'}" title="{$product.name|escape:html:'UTF-8'}"><img class="img-responsive" src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'home_default')|escape:'html'}" alt="{$product.name|escape:html:'UTF-8'}" />
												{if isset($product.new) && $product.specific_prices} 
													{if $product.specific_prices}<span class="sale">{l s='Sale' mod='postabcateslider2'}</span>{/if}
												{else}
												{if isset($product.new) && $product.new == 1}<span class="new">{l s='New' mod='postabcateslider2'}</span>{/if}
												{if $product.specific_prices}<span class="sale">{l s='Sale' mod='postabcateslider2'}</span>{/if}
												{/if}								
											</a>
													...
													<div class="acctions-inner">
														<div class="cart">
															...
															{if isset($static_token)}
																<a class="button ajax_add_to_cart_button btn btn-default" href="{$link->getPageLink('cart',false, NULL, "add=1&amp;id_product={$product.id_product|intval}&amp;token={$static_token}", false)|escape:'html':'UTF-8'}" rel="nofollow"  title="{l s='' mod='postabcategory'}" data-id-product="{$product.id_product|intval}">
																	<span><i class="fa fa-plus-square"></i> {l s='Add to cart' mod='postabcateslider2'}</span>
															...
															<a class="button ajax_add_to_cart_button btn btn-default"  href="{$link->getPageLink('cart',false, NULL, 'add=1&amp;id_product={$product.id_product|intval}', false)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='' mod='postabcategory'}" data-id-product="{$product.id_product|intval}">
																<span>{l s='Add to cart' mod='postabcateslider2'}</span>
															</a>
															   {/if}      
															{else}
														...
														<ul class="add-to-links">
															...
															{if isset($quick_view) && $quick_view}
																<a class="quick-view" data-toggle="tooltip" data-placement="top" data-original-title="{l s='Quickview'  mod='postabcateslider2'}" href="{$product.link|escape:'html':'UTF-8'}" rel="{$product.link|escape:'html':'UTF-8'}">

																</a>
															{/if}
															</li>
															<li>
																<a class="addToWishlist wishlistProd_{$product.id_product|intval}"  data-toggle="tooltip" data-placement="top" data-original-title="{l s='Wishlist' mod='postabcateslider2'}" href="#" data-wishlist="{$product.id_product|intval}" onclick="WishlistCart('wishlist_block_list', 'add', '{$product.id_product|intval}', false, 1); return false;">
																	<span>{l s="Wishlist" mod='postabcateslider2'}</span>
															...
															<li>
																...
																{if isset($comparator_max_item) && $comparator_max_item}
																  <a class="add_to_compare" data-toggle="tooltip" data-placement="top" data-original-title="{l s='Compare' mod='postabcateslider2'}"  href="{$product.link|escape:'html':'UTF-8'}" data-id-product="{$product.id_product}"><i class="fa fa-retweet"></i>{l s='Compare' mod='postabcateslider2'}
				...
				<div class="boxprevnext">
					<a class="prev prevtabcate2"><i class="icon-angle-left"></i></a>
					<a class="next nexttabcate2"><i class="icon-angle-right"></i></a>
				</div>
			</div>
		</div>
	</div>
