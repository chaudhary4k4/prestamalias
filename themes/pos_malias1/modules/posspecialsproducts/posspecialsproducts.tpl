								...
								<div class="actions">
									{if isset($quick_view) && $quick_view}
										<div class="quick_view">
											<a class="quick-view" data-toggle="tooltip" data-placement="top" data-original-title="Quick view" href="{$product.link|escape:'html':'UTF-8'}" rel="{$product.link|escape:'html':'UTF-8'}">
										...
										{if ($product.id_product_attribute == 0 || (isset($add_prod_display) && ($add_prod_display == 1))) && $product.available_for_order && !isset($restricted_country_mode) && $product.minimal_quantity <= 1 && $product.customizable != 2 && !$PS_CATALOG_MODE}
											{if ($product.allow_oosp || $product.quantity > 0)}
												{if isset($static_token)}
													<a class="button ajax_add_to_cart_button btn btn-default" data-toggle="tooltip" data-placement="top" data-original-title="{l s='Add to cart'  mod='posspecialsproducts'}" href="{$link->getPageLink('cart',false, NULL, "add=1&amp;id_product={$product.id_product|intval}&amp;token={$static_token}", false)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Add to cart' mod='posspecialsproducts'}" data-id-product="{$product.id_product|intval}">
														<i class="fa fa-plus-circle"></i>{l s='Add to cart' mod='posspecialsproducts'}
													</a>
												{else}
													<a class="button ajax_add_to_cart_button btn btn-default" data-toggle="tooltip" data-placement="top" data-original-title="{l s='Add to cart'  mod='posspecialsproducts'}" href="{$link->getPageLink('cart',false, NULL, 'add=1&amp;id_product={$product.id_product|intval}', false)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Add to cart' mod='posspecialsproducts'}" data-id-product="{$product.id_product|intval}">
														<i class="fa fa-plus-circle"></i>{l s='Add to cart' mod='posspecialsproducts'}
													</a>
												{/if}						
											{else}
												<span class="button ajax_add_to_cart_button btn btn-default disabled" data-toggle="tooltip" data-placement="top" data-original-title="{l s='Add to cart'  mod='posspecialsproducts'}">
													<i class="fa fa-plus-circle"></i>{l s='Add to cart' mod='posspecialsproducts'}
												</span>
											{/if}
										{/if}
